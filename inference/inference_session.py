...
class nnInteractiveInferenceSession():
    ...
    def set_image(self, np_image: np.ndarray, normalize=True):
        """
        Sets the input image directly for ONNX inference.

        This bypasses any background processing and stores
        the image for direct use by ONNX models.

        Args:
            np_image (np.ndarray): Input image, shape (X, Y, Z) or (C, X, Y, Z)
            normalize (bool): Whether to normalize intensities
        """
        if np_image.ndim == 3:
            np_image = np_image[np.newaxis, ...]  # add channel
        elif np_image.ndim != 4:
            raise ValueError(f"Expected 3D or 4D input, got shape {np_image.shape}")
        ...
        if normalize:
            ...
            if std > 0:
                ...
                print(f"Normalized image (mean={mean:.3f}, std={std:.3f})")
            else:
                print("Warning: std=0, skipping normalization")

        self.preprocessed_image = np_image.astype(np.float32)
        self.original_image_shape = self.preprocessed_image.shape
        print("✅ Image stored for ONNX inference:", self.original_image_shape)
    ...
    def _background_set_image(self, image: np.ndarray, image_properties: dict):
        # Convert and clone the image tensor. It is named image_torch for now, but we'll switch it later.
        ...
        if len(nonzero_idx[0]) == 0:
            raise ValueError("Input image has no nonzero pixels")

        # Create bounding box: for each dimension, get the min and max (plus one) of the nonzero indices.
        bbox = [[int(np.min(ax)), int(np.max(ax)) + 1] for ax in nonzero_idx]
        ...
        if std > 0:
            ...
            print("std is zero, skipping normalization")

        self.preprocessed_image = image_torch
        self.preprocessed_props = {'bbox_used_for_cropping': bbox[1:]}  # coordinates and axis


        # we need to wait for this here I believe
        self.interactions_future.result()
        del self.interactions_future # delete if causing problems.
        self.interactions_future = None
